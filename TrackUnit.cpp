...
// CurrentSpeedButtonTag = 0; //not assigned yet
	...
	RouteFailMessage = "请确认选取【是否超限】或【是否逆向】或【是否连通】！";
        ...
        for(unsigned int y = 0; y < 4; y++) // check all links for each element
        {
            ...
            if(ConnectionFoundFlag)
            {
                ...
                if((TrackElementAt(1157, x).Config[1 - y] == Signal) && IsLCAtHV(50, TrackElementAt(1158, VecPos).HLoc, TrackElementAt(1350, VecPos).VLoc))
                {
                    ...
                    ShowMessage("稍后【单元反色提示】的紧邻位置，不能铺画【挡车器】【交接点】【信号机】单元！");
...
// set element lengths to min of 20m
    for(unsigned int x = 0; x < TrackVector.size(); x++)
    {
        ...
        if((TrackElementAt(1215, x).Length01 < 20) && (TrackElementAt(1435, x).Length01 != -1))
        {
...
// set element lengths to min of 20m
    for(unsigned int x = 0; x < TrackVector.size(); x++)
    {
        ...
        if((TrackElementAt(1285, x).Length01 < 20) && (TrackElementAt(1436, x).Length23 != -1))
        {
...
*/
    ...
    for(PDVIt = (PrefDirVector.begin() + LastIteratorValue); PDVIt < PrefDirVector.end(); PDVIt++)
    {
        ...
        if((PDVIt->TrackType == SignalPost) && (PDVIt->SigAspect == TTrackElement::GroundSignal))
        {
            continue;
        }
...
// throw(Exception("Failed to track prefdir in PresetAutoSigRoutesButtonClick (1)"));  //[GM error 14/04/13] for next release change this to 'continue;' to quit from trying to find the auto route (don't need to throw an exception)
        ...
        while(true)
        {
            ...
            // can't be a gound signal as would have failed the validity test
            {
...
*/
    ...
    {
        TrainController->StopTTClockMessage(80, "【挡车器不能设置为自动闭塞分区的始末位置！】");
    ...
    {
        TrainController->StopTTClockMessage(7, "【进路始末应选择同向信号机！】如果多次无效，请用变通进路！");
        Utilities->CallLogPop(200);
        return(false);
    }
    if(Track->IsLCAtHV(18, HLoc, VLoc))
    {
		TrainController->StopTTClockMessage(73, "【平交道口不能设置为进路始末位置！】");
...
// check if selected a train & disallow if so
    ...
    {
        TrainController->StopTTClockMessage(9, "【列车标签不能设置为进路始末位置！】");
...
// check if selected a locked route element & disallow (can only be a 2-track element so only need check XLinkPos values of 0 & 1
    ...
    {
		TrainController->StopTTClockMessage(10, "【当前进路已被锁定，2分钟后再操作！】");
    ...
    {
		TrainController->StopTTClockMessage(11, "【当前进路已被锁定，2分钟后再操作！】");
...
// look for exact match in a route first - can't be a bridge so can use a simple 'find'
    ...
    if(RoutePair.first > -1) // no need to examine DummyPair as start element can only be in one route (bridges not allowed)
    {
        ...
        {
			TrainController->StopTTClockMessage(13, "【当前位置已设置进路，请勿重复操作！】");
        ...
        {
			TrainController->StopTTClockMessage(14, "【当前位置为辖区移交点！】");
        ...
        {
			TrainController->StopTTClockMessage(15, "【当前位置已设置进路，请勿重复操作！】");
...
// check if selected position is adjacent to start or end of an existing route and disallow
        for(unsigned int a = 0; a < AllRoutes->AllRoutesSize(); a++)
        {
            ...
            {
				TrainController->StopTTClockMessage(16, "【当前位置不能设置为进路起始位置！】");
                Utilities->CallLogPop(210);
                return(false);
            }
            if((StartElement1.Conn[1] > -1) && (StartElement1.Conn[1] == FirstElement.TrackVectorPosition))
            {
                TrainController->StopTTClockMessage(17, "【当前位置不能设置为进路起始位置！】");
...
// check if it's adjacent to end of an an existing route,
        for(unsigned int a = 0; a < AllRoutes->AllRoutesSize(); a++)
        {
            ...
            {
				TrainController->StopTTClockMessage(18, "【当前位置不能设置为进路起始位置！】");
...

{
    ...
    {
		TrainController->StopTTClockMessage(72, "【平交道口不能设置为进路始末位置！】");
...
// cancel selection if on original start element
    ...
    if(AutoSigsFlag)
    {
        ...
        {
            TrainController->StopTTClockMessage(19, "【挡车器不能设置为自动闭塞分区的始末位置！】");
    ...
    {
        TrainController->StopTTClockMessage(20, "【进路始末应选择同向信号机！】如果多次无效，请用变通进路！");
...
// check if train on element
    if(TrackElement.TrainIDOnElement > -1)
    {
		TrainController->StopTTClockMessage(22, "【列车标签不能设置为进路始末位置！】");
...
// bool InRoute = false;
    ...
    if(RoutePair.first > -1)
    {
        ...
        {
			TrainController->StopTTClockMessage(24, "【进路末端应选择同向信号机【或】起始箭头！】");
...
// if((RouteElement.Config[RouteElement.ELinkPos] != End) && (AllRoutes->TrackIsInARoute(, RouteElement.Conn[RouteElement.ELinkPos], RouteElement.ELinkPos)))
        ...
        {
			TrainController->StopTTClockMessage(25, "【当前位置不能设置为进路起始位置！】");
...
// (EndElement1.Conn[EndElement1.XLinkPos] == AdjPosition))
        ...
        {
			TrainController->StopTTClockMessage(26, "【进路末端应选择同向信号机【或】起始箭头！】");
...
// (EndElement2.Conn[EndElement2.XLinkPos] == AdjPosition))
        ...
        {
			TrainController->StopTTClockMessage(27, "【进路末端应选择同向信号机【或】起始箭头！】");
...
// check if adjacent to end of a route & disallow
        ...
        {
            TrainController->StopTTClockMessage(28, "【进路末端应选择同向信号机【或】起始箭头！】");
...
// check for same route as start element
    ...
    {
		TrainController->StopTTClockMessage(29, "【进路末端应选择同向信号机【或】起始箭头！】");
...
// check for a looping route
    if((ReqPosRouteID > -1) && (StartSelectionRouteID > -1))
    {
        ...
        {
			TrainController->StopTTClockMessage(69, "【环线不能设置为闭环进路！】");
...
// need to ensure a signal/buffer/continuation
            ...
            {
                TrainController->StopTTClockMessage(94, "【进路末端应选择同向信号机【或】起始箭头！】"); //added at v2.7.0
            ...
            if(Track->LCFoundInAutoSigsRoute)
            {
                if(!Track->SuppressRouteFailMessage)
                {
                    TrainController->StopTTClockMessage(76, "【平交道口不能设置自动闭塞分区！】");
                    Track->SuppressRouteFailMessage = true;
                }
                for(int x = 0; x < VectorCount; x++)
                {
...
// recursive search at XLinkPos of SearchPos1 (i.e. 'set' trailing exit)
                                           ...
                                           AutoSigsFlag))
                {
                    if(Track->LCFoundInAutoSigsRoute)
                    {
                        if(!Track->SuppressRouteFailMessage)
                        {
                            TrainController->StopTTClockMessage(77, "【平交道口不能设置自动闭塞分区！】");
                            Track->SuppressRouteFailMessage = true;
                        }
                        for(int x = 0; x < VectorCount; x++)
                        {
...
// recursive search at XLinkPos of SearchPos2 (i.e. 'unset' trailing exit)
                                       ...
                                       AutoSigsFlag))
            {
                if(Track->LCFoundInAutoSigsRoute)
                {
                    if(!Track->SuppressRouteFailMessage)
                    {
                        TrainController->StopTTClockMessage(78, "【平交道口不能设置自动闭塞分区！】");
                        Track->SuppressRouteFailMessage = true;
                    }
                    for(int x = 0; x < VectorCount; x++)
                    {
...
*/
    ...
    if((TrackElement.TrackType == Points) || (TrackElement.TrackType == Bridge) || (TrackElement.TrackType == Crossover))
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(34, "【当前位置不能设置为进路起始位置！】");
        }
// makes later adjacent route checks too complicated
    ...
    {
		TrainController->StopTTClockMessage(74, "【平交道口不能设置为进路始末位置！】");
...
// check if selected a train & disallow if so
    if(TrackElement.TrainIDOnElement > -1)
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(35, "【列车标签不能设置为进路始末位置！】");
        }
...
// check if selected a locked route element & disallow (can only be a 2-track element so only need check XLinkPos values of 0 & 1
    ...
    if(AllRoutes->IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(3, TrackVectorPosition, 0, PrefDirElement, LockedVectorNumber))
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(36, "【当前进路已被锁定，2分钟后再操作！】");
        }
    ...
    if(AllRoutes->IsElementInLockedRouteGetPrefDirElementGetLockedVectorNumber(4, TrackVectorPosition, 1, PrefDirElement, LockedVectorNumber))
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(37, "【当前进路已被锁定，2分钟后再操作！】");
        }
...
// look for exact match in a route first - can't be a 3 or 4 track element so only need to look for one TRouteElementPair
    ...
    if(RoutePair.first > -1)
    {
        if(RoutePair.second != AllRoutes->GetFixedRouteAt(31, RoutePair.first).PrefDirSize() - 1) // not last element in existing route so no good
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(38, "【当前位置已设置进路，请勿重复操作！】");
            }
        ...
        if(RouteElement.Conn[RouteElement.XLinkPos] < 0) // last element in existing route but nowhere to go!
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(39, "【当前位置为辖区移交点！】");
            }
        ...
        // last element in existing route but that route linked to another route (or a non-bridge 2-track element containing a route) so no good
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(40, "【当前位置已设置进路，请勿重复操作！】");
            }
...
// check if it's adjacent to start of an an existing route,
        for(unsigned int a = 0; a < AllRoutes->AllRoutesSize(); a++)
        {
            ...
            if((StartElement1.Conn[0] > -1) && (StartElement1.Conn[0] == FirstElement.TrackVectorPosition))
            {
                if(!Callon)
                {
                    TrainController->StopTTClockMessage(41, "【当前位置不能设置为进路起始位置！】");
                }
            ...
            if((StartElement1.Conn[1] > -1) && (StartElement1.Conn[1] == FirstElement.TrackVectorPosition))
            {
                if(!Callon)
                {
                    TrainController->StopTTClockMessage(42, "【当前位置不能设置为进路起始位置！】");
                }
...
// check if it's adjacent to end of an an existing route,
        for(unsigned int a = 0; a < AllRoutes->AllRoutesSize(); a++)
        {
            ...
            if(LastElement.Conn[LastElement.XLinkPos] == StartRoutePosition)
            {
                if(!Callon)
                {
                    TrainController->StopTTClockMessage(43, "【当前位置不能设置为进路起始位置！】");
                }
...
// cancel selection if on original start element
    ...
    {
        TrainController->StopTTClockMessage(75, "【平交道口不能设置为进路始末位置！");
    ...
    if((TrackElement.TrackType == Points) && !Callon)
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(44, "【当前位置不能设置为进路起始位置！】");
        }
// makes later adjacent route checks too complicated
    ...
    if((TrackElement.TrackType == Bridge) || (TrackElement.TrackType == Crossover))
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(71, "【当前位置不能设置为进路起始位置！】");
        }
// makes later adjacent route checks too complicated
...
// check if train on element
    if(TrackElement.TrainIDOnElement > -1)
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(45, "【当前位置不能设置为进路起始位置！】");
        }
...
// bool InRoute = false;
    ...
    if(RoutePair.first > -1)
    {
        if(RoutePair.second != 0) // not first element in existing route so no good
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(46, "【进路末端应选择同向信号机【或】起始箭头！】");
            }
...
// if((RouteElement.Config[RouteElement.ELinkPos] != End) && (AllRoutes->TrackIsInARoute(, RouteElement.Conn[RouteElement.ELinkPos], RouteElement.ELinkPos)))
        ...
        // first element in existing route but that route linked to another route (or a non-bridge 2-track element containing a route) so no good
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(47, "【当前位置不能设置为进路起始位置！】");
            }
...
// && (AdjPosition != StartRoutePosition))
           ...
           (EndElement1.ConnLinkPos[EndElement1.XLinkPos] == AdjLinkPos) && (AdjPosition != StartRoutePosition))
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(48, "【进路末端应选择同向信号机【或】起始箭头！】");
            }
...
// (EndElement2.Conn[EndElement2.XLinkPos] == AdjPosition) && (AdjPosition != StartRoutePosition))
                ...
                (AdjPosition != StartRoutePosition))
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(49, "【进路末端应选择同向信号机【或】起始箭头！】");
            }
...
// check if adjacent to end of a route & disallow (unless end of existing route is the start of this route - i.e. extending route by 1 element)
           ...
           (EndOfRouteElement.TrackVectorPosition != StartRoutePosition))
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(50, "【进路末端应选择同向信号机【或】起始箭头！】");
            }
...
// check for same route as start element
    if((ReqPosRouteID > -1) && (ReqPosRouteID == StartSelectionRouteID))
    {
        if(!Callon)
        {
            TrainController->StopTTClockMessage(51, "【进路末端应选择同向信号机【或】起始箭头！】");
        }
...
// check for a looping route
    if((ReqPosRouteID > -1) && (StartSelectionRouteID > -1))
    {
        if(AllRoutes->CheckForLoopingRoute(1, EndElement1.GetTrackVectorPosition(), EndElement1.GetXLinkPos(), StartElement1.GetTrackVectorPosition()))
        {
            if(!Callon)
            {
                TrainController->StopTTClockMessage(70, "【环线不能设置为闭环进路！】");
            }
...
// above removed at v2.1.0 so that routes can be locked when occupied, below added
            TrainOccupyingRoute = true;  // train is forward of the truncate point
        }
        if(Track->IsLCBarrierFlashingAtHV(3, PrefDirVector.at(b).HLoc, PrefDirVector.at(b).VLoc))
        {
            TrainController->StopTTClockMessage(79, "【平交道口办理作业中，1分钟后再取消进路！】");
            ReturnFlag = InRouteFalse;
    ...
    for(unsigned int b = 0; b < PrefDirSize(); b++)
    {
        if((PrefDirVector.at(b).HLoc == HLoc) && (PrefDirVector.at(b).VLoc == VLoc)) // b = the truncate point
        {
            if(PrefDirVector.at(b).TrackType == Bridge)
            {
				TrainController->StopTTClockMessage(57, "【跨线位置无法取消进路！】");
                ReturnFlag = InRouteFalse;
            ...
            if(b == 1)
            {
				TrainController->StopTTClockMessage(58, "【指针应放在进路起始箭头【右击】鼠标！】");
                ReturnFlag = InRouteFalse;
            ...
            if(b > 0)
            {
                ...
                if(TempElement.PrefDirRoute || TempElement.AutoSignals)
                {
                    ...
                    {
						TrainController->StopTTClockMessage(59, "【指针应放在进路起始箭头【右击】鼠标！】");
                        ReturnFlag = InRouteFalse;
                ...
                else
                {
                    ...
                    {
						TrainController->StopTTClockMessage(60, "【指针应放在进路起始箭头【右击】鼠标！】");
                        ReturnFlag = InRouteFalse;
...
// check if part of this route already locked & disallow if so
            if(!(AllRoutes->LockedRouteVector.empty()))
            {
                for(TAllRoutes::TLockedRouteVectorIterator LRVIT = AllRoutes->LockedRouteVector.begin(); LRVIT < AllRoutes->LockedRouteVector.end(); LRVIT++)
                {
                    ...
                    {
						TrainController->StopTTClockMessage(61, "【当前进路已被锁定，2分钟后再操作！】");
                        ReturnFlag = InRouteFalse;
            ...
            // RouteLockingRequired only checks for trains approaching
            {
                ...
                int button = Application->MessageBox(L"是否强制取消进路【？】【？】【？】",
                                                     L"", MB_YESNO | MB_ICONWARNING);
...
// erase element from route
    GetModifiableRouteAt(8, RequiredRoutePair.first).EraseRouteElementAt(&(GetModifiableRouteAt(9, RequiredRoutePair.first).GetModifiablePrefDirElementAt(1,
                                                                                                                                                          RequiredRoutePair.second)));
...
{
/* Locked if a train moving in the direction of the route within 3 signals back from truncate point (on the route itself or any linked routes, or on the element
...
*/
    Utilities->CallLog.push_back(Utilities->TimeStamp() + "," + AnsiString(Caller) + ",RouteLockingRequired," + AnsiString(RouteNumber) + "," +
                                 AnsiString(RouteTruncatePosition));
    ...
    while(ExamineRoute)
    {
        for(int x = StartPosition; x >= 0; x--)  //work back along the route from the truncate point
        {
            ...
            if(TrainID > -1)
            {
                ...
                return(true);                  //TrainOccupyingRoute which is outside this function but also causes route locking)
            }
    ...
    for(TLockedRouteVectorIterator LRVIT = LockedRouteVector.begin(); LRVIT < LockedRouteVector.end(); LRVIT++)
    {
        ...
        for(int x = GetFixedRouteAt(141, RouteNumber).PrefDirSize() - 1; x >= 0; x--)
        {
            ...
            else if(InternalPrefDirElement.TrackVectorPosition == (int)LRVIT->TruncateTrackVectorPosition)
            {
                ...
                {
                    break; // reached & tested LRVIT->TruncateTrackVectorPosition for a match so don't want to go any further for this route
                }
            }
        }
        VectorCount++;
