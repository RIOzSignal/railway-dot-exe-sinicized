...
*/
//【THIS CPP FILE IS SINICIZED BY RIO≒Signal调度模拟Xeon_HSU】
// ---------------------------------------------------------------------------
#include <Classes.hpp>
...

{
    ...
    // gives a random number between 0 and 32767 (defined as RAND_MAX in stdlib.h)
    {
...
*/
    ...
    if(StoppedForTrainInFront && !StoppedAtLocation && !Crashed && !Derailed && !TrainFailed)
    {
        PlotTrainWithNewBackgroundColour(42, clStoppedTrainInFront, Display);
    }
    if(SignallerStopped && !StoppedForTrainInFront && !StoppedAtBuffers && !StoppedAtLocation && !Crashed && !Derailed && !TrainFailed)
    {
...
*/
    ...
    if(!Crashed)
    {
        if((Straddle == LeadMid) && (LeadElement > -1) && (Track->TrackElementAt(602, LeadElement).TrackType == Buffers))
        {
            ...
            else if(!StoppedAtLocation && !StoppedWithoutPower)
    ...
    if((TrainMode == Timetable) && StoppedAtLocation && (ActionVectorEntryPtr->Command != ""))  //if Command == "" then either TimeLoc or TimeTimeLoc
    {
    ...
    if(TrainMode == Timetable)
    {
        if(!DepartureTimeSet && StoppedAtLocation)
    ...
    if(TrainMode == Timetable)
    {
        ...
        if(HoldAtLocationInTTMode)
        {
                                                                                                      ...
                                                                                                      LastActionTime + TDateTime(30.0 / 86400)))
            {
                ...
                {
                    NewShuttleFromNonRepeatService(0);
                }
                else if(ActionVectorEntryPtr->Command == "Frh-sh")
                {
                    RepeatShuttleOrRemainHere(0);
                }
                else if(ActionVectorEntryPtr->Command == "Fns-sh")
                {
                    RepeatShuttleOrNewNonRepeatService(0);
                }
/*
...
*/
    ...
    if(TrainMode == Timetable)
    {
        if(StoppedAtBuffers)
        {
            ...
            else if((BufferLocation != "") && (BufferLocation == ExpectedLocation) && DepartureTimeSet && !StoppedAtLocation && (TrainController->TTClockTime >
                                                                                                                                 ReleaseTime))
            {
                BufferZoomOutFlashRequired = true;
...
*/
    ...
    if(TrainMode == Timetable)
    {
        if(StoppedAtLocation && !StoppedAtBuffers && !Crashed && !Derailed && !HoldAtLocationInTTMode && !TrainFailed)
        {
            if(BeingCalledOn)
            {
...
*/
    ...
    // During this function train moves fully onto 2 elements, Lead & Mid, so set next 2 moves from here for the element after Lead
    {
        ...
        // not entering at a continuation so can deal with train leaving the lag element
        {
            ...
            if(LeadElement > -1)
            {
                if(Track->TrackElementAt(224, LeadElement).Config[LeadExitPos] == Signal)
...
void TTrain::SetTrainElementID(int Caller, unsigned int TrackVectorPosition, int EntryPos)
{
    ...
    if(Track->TrackElementAt(311, TrackVectorPosition).TrackType == Bridge)
    {
        ...
        if(EntryPos < 2)
        {
            Track->TrackElementAt(312, TrackVectorPosition).TrainIDOnBridgeTrackPos01 = TrainID;
        }
        else
        {
            Track->TrackElementAt(313, TrackVectorPosition).TrainIDOnBridgeTrackPos23 = TrainID;
        }
    }
...
void TTrain::ResetTrainElementID(int Caller, unsigned int TrackVectorPosition, int EntryPos)
{
    ...
    else
    {
        ...
        if(EntryPos < 2)
        {
            Track->TrackElementAt(316, TrackVectorPosition).TrainIDOnBridgeTrackPos01 = -1;
        }
        else
        {
            Track->TrackElementAt(317, TrackVectorPosition).TrainIDOnBridgeTrackPos23 = -1;
        }
        if((EntryPos < 2) && (Track->TrackElementAt(318, TrackVectorPosition).TrainIDOnBridgeTrackPos23 > -1))
        ...
        else if((EntryPos > 1) && (Track->TrackElementAt(321, TrackVectorPosition).TrainIDOnBridgeTrackPos01 > -1))
...
*/{
    ...
    while(true)
    {
        ...
        if((Track->TrackElementAt(387, CurrentTrackVectorPosition).TrackType == Bridge) || (Track->TrackElementAt(388,
                                                                                                                  CurrentTrackVectorPosition).TrackType == Crossover))
        {
        ...
        {
            if((Track->TrackElementAt(525, CurrentTrackVectorPosition).TempTrackMarker01) || (Track->TrackElementAt(526,
                                                                                                                    CurrentTrackVectorPosition).TempTrackMarker23))
            {
                ReturnVal = 4;
                break;
            }
...
*/{
    ...
    if(StoppedAtLocation || !StoppedAtSignal || TimetableFinished || Crashed || Derailed || DerailPending || StoppedAfterSPAD || StoppedForTrainInFront)
...
bool TTrain::IsThereAnAdjacentTrain(int Caller, TTrain *&TrainToBeJoinedBy)
{
    ...
    if(FrontValid)
    {
        if(FrontAdjacentTrackElement.TrackType == Bridge)
        {
            ...
            {
                TrainToBeJoinedByID = FrontAdjacentTrackElement.TrainIDOnBridgeTrackPos23;
            }
            else
            {
                TrainToBeJoinedByID = FrontAdjacentTrackElement.TrainIDOnBridgeTrackPos01;
            }
        }
        else
        {
    ...
    if((TrainToBeJoinedByID < 0) && RearValid)
    {
        ...
        if(RearAdjacentTrackElement.TrackType == Bridge)
        {
            ...
            {
                TrainToBeJoinedByID = RearAdjacentTrackElement.TrainIDOnBridgeTrackPos23;
            }
            else
            {
                TrainToBeJoinedByID = RearAdjacentTrackElement.TrainIDOnBridgeTrackPos01;
            }
        }
        else
        {
...
*/{
    ...
    {
        ActionLog = "【已转为人工调度】【位于】";
    }
    if(ActionType == RestoreTimetableControl)
    {
        ActionLog = "【已恢复自主行车】【位于】";
    }
    if(ActionType == RemoveTrain)
    {
        if(Crashed)
        {
            ActionLog = "【事故列车已被移除出线路】【位于】";
        }
        else if(Derailed)
        {
            ActionLog = "【事故列车已被移除出线路】【位于】";
    ...
    if(Warning)
    {
        BaseLog = Utilities->Format96HHMMSS(ActualTime) + "【故障警报】" + HeadCode + ActionLog + OtherHeadCode + LocationName;
    ...
    if(TimePerformance)
    {
        ...
        {
            PerfLog = "【晚点" + AnsiString(IntMinsLate) + "分】 ";
        }
        else if(IntMinsLate < -1)
        {
            int PosIntMinsLate = -IntMinsLate;
            PerfLog = "【早点" + AnsiString(PosIntMinsLate) + "分】 ";
        }
    ...
    {
        Display->PerformanceLog(1, BaseLog);
    }
    if(Warning)
    {
...
void TTrain::TrainHasFailed(int Caller)
{
    ...
    TrainController->StopTTClockMessage(81, HeadCode + "【列车发生故障】【位于】" + LocName);
    TrainFailed = true;
    TrainFailurePending = false;
...
*/
    ...
    {
        TrainIDOnRearOfRearTrain = RearMostElement.TrainIDOnBridgeTrackPos23;
    }
    else if((RearMostElement.TrackType == Bridge) && (RearTrainExitPos < 2))
    {
        TrainIDOnRearOfRearTrain = RearMostElement.TrainIDOnBridgeTrackPos01;
    }
    else
    {
    ...
    {
        TrainIDOnFrontOfFrontTrain = FrontMostElement.TrainIDOnBridgeTrackPos23;
    }
    else if((FrontMostElement.TrackType == Bridge) && (FrontTrainExitPos < 2))
    {
        TrainIDOnFrontOfFrontTrain = FrontMostElement.TrainIDOnBridgeTrackPos01;
    }
    else
    {
...
void TTrain::NewTrainService(int Caller)
...
void TTrain::RemainHere(int Caller)
{
    ...
    {
        Display->PerformanceLog(5, Utilities->Format96HHMMSS(TrainController->TTClockTime) + "  " + HeadCode + "【已结束运营】" +
...
bool TTrain::AbleToMove(int Caller)
{
    ...
    if(LeadElement > -1)
    {
        ...
        {
            if(Track->TrackElementAt(801, LeadElement).TrackType == Buffers)
            {
                Able = false;
            }
...
AnsiString TTrain::FloatingLabelNextString(int Caller, TActionVectorEntry *Ptr)
{
    ...
    if(Ptr->FormatType == TimeTimeLoc)
    {
        if(TrainMode == Timetable)
        {
            ...
            {
				RetStr = "" + Ptr->LocationName + "【图定到点】" + Utilities->Format96HHMM(GetTrainTime(2, Ptr->ArrivalTime));
            }
            else
            {
				RetStr = "" + Ptr->LocationName + "【图定开点】" + Utilities->Format96HHMM(GetTrainTime(3, Ptr->DepartureTime));
            }
        }
        else // TrainMode == Signaller
        {
            ...
            {
				RetStr = "" + Ptr->LocationName + "【图定到点】" + Utilities->Format96HHMM(GetTrainTime(45, Ptr->ArrivalTime));
            }
            else
            {
				RetStr = "" + Ptr->LocationName + "【图定开点】" + Utilities->Format96HHMM(GetTrainTime(36, Ptr->DepartureTime));
            }
        }
    }
    else if((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime != TDateTime(-1)))
    {
		RetStr = "" + Ptr->LocationName + "【图定到点】" + Utilities->Format96HHMM(GetTrainTime(4, Ptr->ArrivalTime));
    }
    else if((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime == TDateTime(-1)))
    {
		RetStr = "" + Ptr->LocationName + "【图定开点】" + Utilities->Format96HHMM(GetTrainTime(5, Ptr->DepartureTime));
    }
    else if(Ptr->FormatType == PassTime) //must come after 'else if((Ptr->FormatType == PassTime) && TreatPassAsTimeLocDeparture)'
    {
        RetStr = "" + Utilities->Format96HHMM(GetTrainTime(31, Ptr->EventTime)) + "【图定通过】" + Ptr->LocationName;
    }
    else if(Ptr->Command == "Fns")
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(6, Ptr->EventTime)) + "【变更车次】" + TrainController->GetRepeatHeadCode(8, Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "" +
            "";
    ...
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(32, Ptr->EventTime)) + "【变更车次】" + Ptr->NonRepeatingShuttleLinkHeadCode + "" + ""
            ;
    ...
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(7, Ptr->EventTime)) + "【变更车次】" + TrainController->GetRepeatHeadCode(9, Ptr->OtherHeadCode, RepeatNumber+1, IncrementalDigits) + "" +
            "";
    ...
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(8, Ptr->EventTime)) + "【变更车次】" + Ptr->NonRepeatingShuttleLinkHeadCode
        + "" + "";
    ...
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(9, Ptr->EventTime)) + "【变更车次】" + TrainController->GetRepeatHeadCode(10, Ptr->OtherHeadCode, RepeatNumber+1, IncrementalDigits) + "" +
            "";
        // use RepeatNumber+1 as it's the repeat number of the NEXT shuttle service that is relevant
    ...
    else if(Ptr->Command == "Fer")
    {
		...
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(10, Ptr->EventTime)) + "【准备移交】" + TrainController->GetExitLocationAndAt(1, Ptr->ExitList, AllowedExits) + AllowedExits;
    }
    else if(Ptr->Command == "Fjo")
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(11, Ptr->EventTime)) + "【准备联挂】" + TrainController->GetRepeatHeadCode(11, Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "" + ""
            ;
    }
    else if(Ptr->Command == "jbo")
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(12, Ptr->EventTime)) + "【准备联挂】" + TrainController->GetRepeatHeadCode(12, Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "" +
            "";
    }
    else if(Ptr->Command == "fsp")
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(13, Ptr->EventTime)) + "【准备解编】" + TrainController->GetRepeatHeadCode(13, Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "" +
            "";
    }
    else if(Ptr->Command == "rsp")
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(14, Ptr->EventTime)) + "【准备解编】" + TrainController->GetRepeatHeadCode(14, Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "" +
            "";
    }
    else if(Ptr->Command == "cdt")
    {
		RetStr = "" + Utilities->Format96HHMM(GetTrainTime(15, Ptr->EventTime)) + "【准备调向】";
    }
...
AnsiString TTrain::CheckNewServiceDepartureTime(int Caller, TActionVectorEntry *Ptr, int RptNum, TTrainDataEntry *LinkedTrainDataPtr, AnsiString RetStr)
{
    ...
    for(TActionVectorIterator AVI = NewServiceAV.begin(); AVI < NewServiceAV.end(); AVI++)
    {
        ...
        if(AVI->Command == "jbo")
        {
			...
			RetStr += "【待挂】" + AVI->OtherHeadCode + "【预计】" + EventTime;
    ...
    do
    {
        ...
        if((Ptr->FormatType == TimeTimeLoc) && FirstPass)
        {
            ...
            if(TrainMode == Timetable)
            {
                ...
                {
					PartStr = Utilities->Format96HHMM(GetTrainTime(33, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
                }
                else if(Ptr->ArrivalTime == Ptr->DepartureTime)
                {
					PartStr = Utilities->Format96HHMM(GetTrainTime(34, Ptr->ArrivalTime)) + "【到发】" + Ptr->LocationName;
                }
                else
                {
					PartStr = Utilities->Format96HHMM(GetTrainTime(16, Ptr->ArrivalTime)) + "【到点】" + Ptr->LocationName + '\n' +
                        Utilities->Format96HHMM(GetTrainTime(17, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
            ...
            else // TrainMode == Signaller
            {
                if(DepartureTimeSet)
                {
					PartStr = Utilities->Format96HHMM(GetTrainTime(37, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
                }
                else if(Ptr->ArrivalTime == Ptr->DepartureTime)
                {
					PartStr = Utilities->Format96HHMM(GetTrainTime(38, Ptr->ArrivalTime)) + "【到发】" + Ptr->LocationName;
                }
                else
                {
					PartStr = Utilities->Format96HHMM(GetTrainTime(39, Ptr->ArrivalTime)) + "【到点】" + Ptr->LocationName + '\n' +
                        Utilities->Format96HHMM(GetTrainTime(40, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
                    Count++; // because there are 2 entries
                }
            }
        }
        else if((Ptr->FormatType == TimeTimeLoc) && !FirstPass)
        {
            ...
            {
				PartStr = Utilities->Format96HHMM(GetTrainTime(41, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
            }
            else if(Ptr->ArrivalTime == Ptr->DepartureTime)
            {
				PartStr = Utilities->Format96HHMM(GetTrainTime(42, Ptr->ArrivalTime)) + "【到发】" + Ptr->LocationName;
            }
            else
            {
				PartStr = Utilities->Format96HHMM(GetTrainTime(43, Ptr->ArrivalTime)) + "【到点】" + Ptr->LocationName + '\n' +
                    Utilities->Format96HHMM(GetTrainTime(44, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
                Count++; // because there are 2 entries
            }
        }
        else if((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime != TDateTime(-1)))
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(18, Ptr->ArrivalTime)) + "【到点】" + Ptr->LocationName;
        }
        else if((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime == TDateTime(-1)))
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(19, Ptr->DepartureTime)) + "【开点】" + Ptr->LocationName;
        }
        else if(Ptr->FormatType == PassTime) // new
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(30, Ptr->EventTime)) + "【通过】" + Ptr->LocationName;
        }
        else if(Ptr->Command == "Fns")
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(20, Ptr->EventTime)) + "【变更车次】" + TrainController->GetRepeatHeadCode(15,
        ...
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(35, Ptr->EventTime)) + "【变更车次】" + Ptr->NonRepeatingShuttleLinkHeadCode + " "
                ;
        ...
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(21, Ptr->EventTime)) + "【变更车次】" + TrainController->GetRepeatHeadCode(16,
        ...
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(22, Ptr->EventTime)) + "【变更车次】" + Ptr->NonRepeatingShuttleLinkHeadCode 
            + "";
        ...
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(23, Ptr->EventTime)) + "【变更车次】" + TrainController->GetRepeatHeadCode(17,
                                                                                                                                             Ptr->OtherHeadCode, RepeatNumber + 1, IncrementalDigits) + "";
            // use RepeatNumber+1 because it's the repeat number of the NEXT shuttle service that is relevant
        ...
        {
            AnsiString AllowedExits = "";
            PartStr = Utilities->Format96HHMM(GetTrainTime(24, Ptr->EventTime)) + "【移交到】" + TrainController->GetExitLocationAndAt(2, Ptr->ExitList, AllowedExits) + AllowedExits;
        }
        else if(Ptr->Command == "Fjo")
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(25, Ptr->EventTime)) + "【联挂】" + TrainController->GetRepeatHeadCode(18, Ptr->OtherHeadCode,
                                                                                                                                 RepeatNumber, IncrementalDigits) + "";
        }
        else if(Ptr->Command == "jbo")
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(26, Ptr->EventTime)) + "【联挂】" + TrainController->GetRepeatHeadCode(19, Ptr->OtherHeadCode,
                                                                                                                                      RepeatNumber, IncrementalDigits) + "";
        }
        else if(Ptr->Command == "fsp")
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(27, Ptr->EventTime)) + "【解编】" + TrainController->GetRepeatHeadCode(20,
                                                                                                                                           Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "";
        }
        else if(Ptr->Command == "rsp")
        {
			PartStr = Utilities->Format96HHMM(GetTrainTime(28, Ptr->EventTime)) + "【解编】" + TrainController->GetRepeatHeadCode(21,
                                                                                                                                          Ptr->OtherHeadCode, RepeatNumber, IncrementalDigits) + "";
        }
        else if(Ptr->Command == "cdt")
        {
            PartStr = Utilities->Format96HHMM(GetTrainTime(29, Ptr->EventTime)) + "【调向】" + Ptr->LocationName;
        }
        if(RetStr != "")
        {
            RetStr = RetStr + '\n' + PartStr;
        ...
        for(int x = TrainVector.size() - 1; x >= 0; x--) // reverse because of erase
        {
            ...
            if(Train.HasTrainGone())
            {
                ...
                if(ElementFound)
                {
                    if(TE.ActiveTrackElementName != "")
                    {
						Loc = TE.ActiveTrackElementName + "【位于】" + TE.ElementID;
                    }
                    else
                    {
						Loc ="【位于】" + TE.ElementID;
...
// return true if find the train (added at v2.4.0 as can select a removed train
// in OAListBox before it updates - reported by LiWinDom in error report 23/04/20)
{
...
// Enter with Ptr pointing to first action to be listed (i.e. next action)
{
    ...
    do
    {
        ...
        if(Ptr->FormatType == TimeTimeLoc)
        {
            ...
            {
				PartStr = Utilities->Format96HHMM(GetControllerTrainTime(0, Ptr->ArrivalTime, RepNum, IncMins)) + "【图定到发】" + Ptr->LocationName;
            }
            else
            {
				PartStr = Utilities->Format96HHMM(GetControllerTrainTime(1, Ptr->ArrivalTime, RepNum, IncMins)) + "【图定到点】" + Ptr->LocationName + '\n' +
                    Utilities->Format96HHMM(GetControllerTrainTime(2, Ptr->DepartureTime, RepNum, IncMins)) + "【图定开点】" + Ptr->LocationName;
                Count++; // because there are 2 entries
            }
        }
        else if((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime != TDateTime(-1)))
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(3, Ptr->ArrivalTime, RepNum, IncMins)) + "【图定到点】" + Ptr->LocationName;
        }
        else if((Ptr->FormatType == TimeLoc) && (Ptr->ArrivalTime == TDateTime(-1)))
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(4, Ptr->DepartureTime, RepNum, IncMins)) + "【图定开点】" + Ptr->LocationName;
        }
        else if(Ptr->FormatType == PassTime) // new
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(5, Ptr->EventTime, RepNum, IncMins)) + "【图定通过】" + Ptr->LocationName;
        }
        else if(Ptr->Command == "Fns")
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(6, Ptr->EventTime, RepNum, IncMins)) + "【变更车次】" +
        ...
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(7, Ptr->EventTime, RepNum, IncMins)) + "【变更车次】" +
...
//Since this is a new continuation entry service it can't be Fns-sh or Frh-sh but leave these in for consistency with TTrain::FloatingTimetableString
        ...
        {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(8, Ptr->EventTime, RepNum, IncMins)) + "【变更车次】" +
        ...
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(9, Ptr->EventTime, RepNum, IncMins)) + "【变更车次】" +
                Ptr->NonRepeatingShuttleLinkHeadCode, + "";
        ...
        {
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(10, Ptr->EventTime, RepNum, IncMins)) + "【变更车次】" +
        ...
        {
            AnsiString AllowedExits;
            PartStr = Utilities->Format96HHMM(GetControllerTrainTime(11, Ptr->EventTime, RepNum, IncMins)) + "【准备移交】" +
                TrainController->GetExitLocationAndAt(3, Ptr->ExitList, AllowedExits) + AllowedExits;
        }
        else if(Ptr->Command == "Fjo")
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(12, Ptr->EventTime, RepNum, IncMins)) + "【准备联挂】" + TrainController->GetRepeatHeadCode(49,
                                                                                                                                                            Ptr->OtherHeadCode, RepNum, IncDig) + "";
        }
        else if(Ptr->Command == "jbo")
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(13, Ptr->EventTime, RepNum, IncMins)) + "【准备联挂】" + TrainController->GetRepeatHeadCode
                    (50, Ptr->OtherHeadCode, RepNum, IncDig) + "";
        }
        else if(Ptr->Command == "fsp")
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(14, Ptr->EventTime, RepNum, IncMins)) + "【准备解编】" +
                TrainController->GetRepeatHeadCode(51, Ptr->OtherHeadCode, RepNum, IncDig) + "";
        }
        else if(Ptr->Command == "rsp")
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(15, Ptr->EventTime, RepNum, IncMins)) + "【准备解编】" +
                TrainController->GetRepeatHeadCode(52, Ptr->OtherHeadCode, RepNum, IncDig) + "";
        }
        else if(Ptr->Command == "cdt")
        {
			PartStr = Utilities->Format96HHMM(GetControllerTrainTime(16, Ptr->EventTime, RepNum, IncMins)) + "【准备调向】" + Ptr->LocationName;
        }
        if(RetStr != "")
        {
            RetStr = RetStr + '\n' + PartStr;
...
*/
    ...
    // SaveTrainDataVectorToFile(0);//test
...
/*    else if(ActionEventType == FailBuffersPreventingStart)
    ...
    {
		ErrorLog = "【列车发生碰撞】" + OtherHeadCode + "【位于】";
		Prefix = "【事故警报】";
		WarningStr = "【列车发生碰撞】【位于】";
		Display->WarningLog(4, TimeAndHeadCode + WarningStr + LocationID + '\t' + '\t');
        CrashedTrains++;
	...
	{
		ErrorLog = "【列车发生故障】" + OtherHeadCode + "【位于】";
		Prefix = "【故障警报】";
//        WarningStr = "【列车发生故障】【位于】";
//        Display->WarningLog(5, TimeAndHeadCode + WarningStr + LocationID + '\t' + '\t');
	}
...
//        Prefix = " SPAD RISK: ";
    ...
    {
		Prefix = "【运行提示】";
		ErrorLog = "【股道安排错误】" + OtherHeadCode + "【位于】";
...
//        Display->WarningLog(8, TimeAndHeadCode + WarningStr + LocationID);
    ...
    {
		Prefix = "【运行提示】";
		ErrorLog = "【列车申请联挂】" + OtherHeadCode + "【位于】";
...
/*     print out TrainDataVectorCopy for debugging purposes
    ...
    TTrainDataEntry SingleService = SingleServiceVector.at(SSVectorNumber);
        ...
        for(unsigned int x = 0; x < SingleService.ActionVector.size(); x++)
        {
            ...
            if(AVE.FormatType == StartNew)
            {
                ...
                VecFile << Marker << AnsiString(AVE.EventTime.TimeString()) << ' ' << AVE.Command << ' ' << RearID << ' ' << FrontID << '\n';
            }
            if(AVE.FormatType == SNTShuttle)
            {
                ...
                VecFile << Marker << AnsiString(AVE.EventTime.TimeString()) << ' ' << AVE.Command << ' ' << RearID << ' ' << FrontID << ' ' << AVE.OtherHeadCode << '\n';
            }
            if(AVE.FormatType == SNSShuttle) //should all have been converted to chr
            {
                VecFile << Marker << AnsiString(AVE.EventTime.TimeString()) << ' ' << AVE.Command << ' ' << AVE.OtherHeadCode << ' ' << AVE.NonRepeatingShuttleLinkHeadCode << '\n';
            }
            ...
            if((AVE.FormatType == TimeCmd) || (AVE.FormatType == TimeCmdHeadCode) || (AVE.FormatType == FNSNonRepeatToShuttle) || (AVE.FormatType == FSHNewService))
            {
                ...
                VecFile << Marker << AnsiString(AVE.EventTime.TimeString()) << ' ' << AVE.Command << ' ' << AVE.OtherHeadCode << '\n';
                AVE = AVHolder;
            }
            else if((AVE.FormatType == TimeLoc) && (AVE.ArrivalTime != TDateTime(-1)))
            {
                VecFile << Marker << AnsiString(AVE.ArrivalTime.TimeString()) << " Arr " << AVE.LocationName << '\n';
            }
            else if((AVE.FormatType == TimeLoc) && (AVE.DepartureTime != TDateTime(-1)))
            {
                VecFile << Marker << AnsiString(AVE.DepartureTime.TimeString()) << " Dep " << AVE.LocationName << '\n';
            }
            else if(AVE.FormatType == TimeTimeLoc)
            {
                VecFile << Marker << AnsiString(AVE.ArrivalTime.TimeString()) << ' ' << AnsiString(AVE.DepartureTime.TimeString()) << ' ' << AVE.LocationName << '\n';
            }
            else if(AVE.FormatType == PassTime)
            {
...
void TTrainController::SendPerformanceSummary(int Caller, std::ofstream &PerfFile)
{
    ...
    if(OnTimeArrivals != 1)
    {
        PerfFile << "【正点到达】" << OnTimeArrivals << " 次" << '\t';
    }
    else
    {
        PerfFile << "【正点到达】" << OnTimeArrivals << " 次" << '\t';
    ...
    if(LateArrivals > 1)
    {
        PerfFile << "【晚点】" << LateArrivals << " 次【平均 " << AvLateArrMins.c_str() << " 分钟】" << '\t';
    }
    ...
    {
        PerfFile << "【晚点】" << LateArrivals << " 次" << '\t';
    }
    if(EarlyArrivals > 1)
    {
        PerfFile << "【早点】" << EarlyArrivals << " 次【平均 " << AvEarlyArrMins.c_str() << " 分钟】" << '\n';
    }
    ...
    {
        PerfFile << "【早点】" << EarlyArrivals << " 次" << '\n';
    }
    if(OnTimeDeps != 1)
    {
        PerfFile << "【正点发车】" << OnTimeDeps << " 次" << '\t';
    }
    else
    {
        PerfFile << "【正点发车】" << OnTimeDeps << " 次" << '\t';
    ...
    if(LateDeps > 1)
    {
        PerfFile << "【晚点】" << LateDeps << " 次【平均 " << AvLateDepMins.c_str() << " 分钟】" << '\t' <<"【早点】0 次" << '\n';
    }
    else if(LateDeps == 1)
    {
        PerfFile << "【晚点】" << LateDeps << " 次【平均 " << AvLateDepMins.c_str() << " 分钟】" << '\t' <<"【早点】0 次" << '\n';
    ...
    {
        PerfFile << "【晚点】" << LateDeps << " 次" << '\t' <<"【早点】0 次" << '\n';
    }
    if(OnTimePasses != 1)
    {
        PerfFile << "【正点通过】" << OnTimePasses << " 次" << '\t';
    }
    else
    {
        PerfFile << "【正点通过】" << OnTimePasses << " 次" << '\t';
    ...
    if(LatePasses > 1)
    {
        PerfFile << "【晚点】" << LatePasses << " 次【平均 " << AvLatePassMins.c_str() << " 分钟】" << '\t';
    }
    ...
    {
        PerfFile << "【晚点】" << LatePasses << " 次" << '\t';
    }
    if(EarlyPasses > 1)
    {
        PerfFile << "【早点】" << EarlyPasses << " 次【平均 " << AvEarlyPassMins.c_str() << " 分钟】" << '\n';
    }
    ...
    {
        PerfFile << "【早点】" << EarlyPasses << " 次" << '\n';
    }

    if(OnTimeExits != 1)        //this batch added at v2.9.1
    {
        PerfFile << "【正点移交】" << OnTimeExits << " 次" << '\t';
    }
    else
    {
        PerfFile << "【正点移交】" << OnTimeExits << " 次" << '\t';
    ...
    if(LateExits > 1)
    {
        PerfFile << "【晚点】" << LateExits << " 次【平均 " << AvLateExitMins.c_str() << " 分钟】" << '\t';
    }
    else if(LateExits == 1)
    {
        PerfFile << "【晚点】" << LateExits << " 次【平均 " << AvLateExitMins.c_str() << " 分钟】" << '\t';
    ...
    {
        PerfFile << "【晚点】" << LateExits << " 次" << '\t';
    }
    if(EarlyExits > 1)
    {
        PerfFile << "【早点】" << EarlyExits << " 次【平均 " << AvEarlyExitMins.c_str() << " 分钟】" << '\n';
    }
...
*/
    ...
    {
        PerfFile << "【平交道口超时】" << FormattedExcessLCDownMins.c_str() << " 分钟" << '\n';
    }
    if(NumFailures != 1)
    {
        PerfFile << "【列车随机故障】" << NumFailures << " 次" << '\n';
    }
    else
    {
        PerfFile << "【列车随机故障】" << NumFailures << " 次" << '\n';
    ...
    if(IncorrectExits != 1)
    {
        PerfFile << "【列车移交错误】" << IncorrectExits << " 次" << '\n';
    }
    else
    {
        PerfFile << "【列车移交错误】" << IncorrectExits << " 次" << '\n';
    ...
    {
        PerfFile << "【列车发生甩站】" << UnexpectedExits << " 次" << '\n';
    }
    else
    {
        PerfFile << "【列车发生甩站】" << UnexpectedExits << " 次" << '\n';
    ...
    {
        PerfFile << "【作业缺失】" << OtherMissedEvents << " 个" << '\n';
    }
    else
    {
        PerfFile << "【作业缺失】" << OtherMissedEvents << " 个" << '\n';
...
/*    if(LocsNotReached > 0)
    ...
    if(CrashedTrains != 1)
    {
        PerfFile << "【列车碰撞事故】" << CrashedTrains << " 次" << '\n';
    }
    else
    {
        PerfFile << "【列车碰撞事故】" << CrashedTrains << " 次" << '\n';
    ...
    // flag condits added at v1.1.4 - see above for what the error was
    {
		...
		PerfFile << "\n【系统评分】" << OverallScorePercent << " 分\n";
        AnsiString Rating = "";
        if(OverallScorePercent > 78)
        {
            Rating = "≒调度完成赢取【10倍】奖励金≒";
        }
...
void TTrainController::CalcOperatingAndNotStartedTrainLateness(int Caller)
{
    ...
    for(unsigned int x = 0; x < TrainVector.size(); x++)
    {
            ...
            AVEntryPtr++)
        {
            ...
            if((AVEntryPtr->ArrivalTime > TDateTime(-1)) && !Train.StoppedAtLocation && (GetRepeatTime(42, AVEntryPtr->ArrivalTime, Train.RepeatNumber, Train.IncrementalMinutes) <
...
// aren't used - this means there is no display for the train in question
{
    ...
    // ok if autosig route after red signal
    {
        ...
        // give 'NOW' indication after allowed to pass red signal when LeadMidLag (AllowedToPassRedSignal reset by this point)
        {
    ...
    // not red signal next (in fwd direction) so enter loop to calc CumLength
    {
        ...
        else
        {
            if(CurrentEntryPos > 1)
            {
                TrainOnElement = TE.TrainIDOnBridgeTrackPos23;
            }
            else
            {
                TrainOnElement = TE.TrainIDOnBridgeTrackPos01;
            }
        }
